...
/*
    ...
    has returned. A closure is created when a function is defined
...
/*
    ...
    Function keyword --> function funcName() {}
    Function expression --> const myFunction = function() {}
    Arrow key --> const myFunction = () => {}
*/


/*
    Question: Describe the event loop in Javascript and its role
    in handling asynchronus code?

    Answer: The event loop is a mechanism in JavaScript that
    handles the execution of code and manages the callback queue.
    It processes the messages or events in the queue and executes
    the corresponding code. The event loop helps handle the
    execution of asynchronous code.
*/


/*
    Question: How do you manipulate the Document Object Model 
    (DOM)

    Answer: The Document Object Model (DOM) can be manipulated
    using JavaScript through its API, which provides methods and 
    properties to access and modify HTML elements and their 
    content.
*/
...
/*
    ...
    Answer: Errors and exceptions can be handled in JavaScript
    using try-catch statements. The code that might throw an error
    is put inside a try block, and the catch block is used to
    handle the error. The catch block takes an error object as an
    argument, which provides information about the error that was thrown.
*/
...
/*
    ...
    meaning that the next line of code will not be executed until
    the current line of code has finished executing. Asynchronous
    code, on the other hand, does not block the execution of the
    rest of the code and allows multiple events to be processed
    simultaneously.
*/
...
/*
    ...
    Answer: null is a value that represents the intentional 
    absence of any object value, while undefined is a value that
    indicates that a variable has been declared but has not been 
    assigned a value.
*/


/*
    Question: Can you explain how prototype-based inheritance
    works in JavaScript?

    Answer: Prototype-based inheritance in JavaScript is a way of
    sharing properties and methods among objects without the need
    for a class. In JavaScript, objects can inherit properties and
